---
title: "Interesting Facts About JoRap (Scraped From My Own Blog)"
meta_title: "Facts About JoRap - A Table From Every Blog Post"
description: "I pulled personal facts from every post on this site into one table, with links back to where I said each thing. A sanity check that the blog still sounds like me."
slug: "interesting-facts-about-jorap"
date: "2026-07-15T18:30:00Z"
image: "/images/joraps-world.jpg"
categories: ["Ideas", "Website"]
author: "JoRap"
tags: ["JoRap", "Personal", "Blogging", "Meta", "Digital Garden", "Identity", "Archive"]
related_notes:
  - drafting-in-public
  - commonplace-book
featured: false
draft: true
---

Every so often I wonder if this site still sounds like me, or like a pile of posts that happen to share a byline. So I did the obvious thing: I read my own archive and pulled out **facts about JoRap** - the person, not the brand - into tables below.

Each row links to the post where I said it. If something feels off, that's the point. Either I changed, or I forgot I wrote it.

Draft posts show *(draft)* in the list.

---

## Pia, Isaiah, and Friday arroz caldo

| Fact | Where I said it |
| :--- | :--- |
| My wife is **Pia**. Our family nickname is **Team Rapusas**. | [The Miracle of Isaiah David](/blog/miracle-isaiah-david/) |
| On **May 8, 2020**, Pia and I found out she was pregnant and gave birth to our son **Isaiah David** the same day - during strict **ECQ** lockdown in the Philippines. | [The Miracle of Isaiah David](/blog/miracle-isaiah-david/) |
| Isaiah was born at **4:46 PM** via C-section, full-term at **3.2 kg**, APGAR **9**. I briefly announced the wrong gender before the doctors corrected me. | [The Miracle of Isaiah David](/blog/miracle-isaiah-david/) |
| Our birth story reached **GMA News Online**, **24 Oras**, **The 700 Club Asia**, and **Smart Parenting**. | [The Miracle of Isaiah David](/blog/miracle-isaiah-david/) |
| I have a **six-year-old son** in grade 1. He reads chapter books and chose **Kumon reading** over Kumon math. | [Kumon for the Reps](/blog/grade-1-better-than-kumon/) *(draft)* |
| I did a **Kumon math trial** years ago. My answers were right; the feedback was **you could do it faster**. | [Kumon for the Reps](/blog/grade-1-better-than-kumon/) *(draft)* |
| I'm **good at math** and didn't want to hand the subject off to a franchise center for my son. | [Kumon for the Reps](/blog/grade-1-better-than-kumon/) *(draft)* |
| I used to make **arroz caldo most Friday nights** in a **Hanabishi rice cooker**, then switched to an **8-quart Instant Pot**. | [Set It and Forget It Arroz Caldo](/blog/set-it-forget-it-arroz-caldo/), [Instant Pot Arroz Caldo](/blog/instant-pot-arroz-caldo/) |
| My go-to **chicken adobo** is tuned for an 8-quart Instant Pot and about **2 kg of bone-in thighs**. | [Instant Pot Chicken Adobo](/blog/instant-pot-chicken-adobo/) |
| I **sear adobo in a separate pan** first - browning in the Instant Pot is how I've gotten burn notices. | [Instant Pot Chicken Adobo](/blog/instant-pot-chicken-adobo/) |

---

## Freelance WordPress, Hugo on Cloudflare

| Fact | Where I said it |
| :--- | :--- |
| I **freelance** and build **client WordPress sites**. About **80% of what I build** repeats the same patterns. | [Consistent AI Output for WordPress Builds](/blog/consistent-ai-output-wordpress-builds/), [Static Site or CMS?](/blog/static-site-vs-cms/) |
| I **taught seminars** on open-source technology and web careers in the **Philippines**. | [Why I Focused on Open Source](/blog/why-i-focused-on-open-source/) |
| My open-source filter: **can I inspect it, move it, and find help when I'm stuck?** | [Why I Focused on Open Source](/blog/why-i-focused-on-open-source/) |
| I compared **Joomla, WordPress, and Drupal**, picked Drupal for architecture, then switched to **WordPress** because I could not find paid work with Drupal in my market. | [Why I Switched from Drupal to WordPress](/blog/drupal-to-wordpress/) |
| I attended **WordCamp Asia in Manila** (~1,400 people) and heard Matt Mullenweg tell old **Philippines WordCamp** stories. | [Why I Switched from Drupal to WordPress](/blog/drupal-to-wordpress/) |
| I lost **shared PHP hosting** when a client moved on, then migrated to **Hugo + GitHub + Cloudflare Pages**. | [How I Built JoRap Notes](/blog/how-i-built-jorap-notes/), [Sustainable Online Publishing](/blog/sustainable-online-publishing/) |
| A git push to this site goes live in about **ninety seconds**. | [How I Built JoRap Notes](/blog/how-i-built-jorap-notes/), [Static Site or CMS?](/blog/static-site-vs-cms/) |
| I pin **`HUGO_VERSION`** on Cloudflare to match local Hugo - mismatch is the most boring deploy failure. | [How I Built JoRap Notes](/blog/how-i-built-jorap-notes/) |
| Hugo theme folder names are **case-sensitive** (`hugoplate` not `HugoPlate`). I learned that the embarrassing way. | [How I Built JoRap Notes](/blog/how-i-built-jorap-notes/) |
| This site runs on **Hugoplate**, mostly for **Tailwind CSS**. I lost a Saturday picking it. | [HugoPlate Theme Review](/blog/hugoplate-theme-review/), [How I Built JoRap Notes](/blog/how-i-built-jorap-notes/) |
| I met **Inter** in *Refactoring UI* and run it as the **only font** on jorap.com - self-hosted, not from Google at page load. | [Inter Font](/blog/inter-font/) *(draft)* |
| I use **Cursor** for AI-assisted WordPress builds and keep a short **rules doc** so output stays consistent. | [Consistent AI Output for WordPress Builds](/blog/consistent-ai-output-wordpress-builds/) |
| **JoRap Notes** (linked notes garden) lives on the same Hugo build as the blog - not a separate wiki host. | [How I Built JoRap Notes](/blog/how-i-built-jorap-notes/), [JoRap Notes Features](/blog/jorap-notes-features/) *(draft)* |
| I quote client sites as **separate line items** (discovery, build, migration, hosting) - no bundled mystery rent. | [How I Quote Client Websites](/blog/selling-static-sites/) *(draft)* |
| I run **jorap.com** because social platforms are a lobby, not a library. | [Why I Still Run My Own Website](/blog/why-run-your-own-website/), [Sustainable Online Publishing](/blog/sustainable-online-publishing/) |

---

## Earphones in a drawer, a cord in the bag

| Fact | Where I said it |
| :--- | :--- |
| I keep a **laptop** because I get real work done faster there - upright, keyboard, big screen. I don't lend it out; I use it all day. | [Why I Still Keep a Desktop or Laptop](/blog/why-keep-desktop-laptop/) |
| **Phone for quick hits. Laptop for anything** that needs a keyboard, a big screen, or more than ten focused minutes. | [Why I Still Keep a Desktop or Laptop](/blog/why-keep-desktop-laptop/) |
| A **short extension cord** (~2 m) lives in my laptop bag for hotel outlets and blocked café sockets. | [The Short Extension Cord in My Laptop Bag](/blog/extension-cord/) |
| In the bag I carry a **plain extension cord**; the **surge strip stays at the desk**. | [The Short Extension Cord in My Laptop Bag](/blog/extension-cord/) |
| I keep a **second cellphone** - underpowered, in a drawer - with **bank apps** on it, away from daily-driver use. | [2nd Cellphone](/blog/second-cellphone/) *(draft)* |
| Most wireless earphones I've bought ended up in a **drawer within a month**. | [What I Look For in Wireless Earphones](/blog/what-i-look-for-in-wireless-earphones/) |
| I want **open-ear** earphones partly to hear **jeepneys** (and my kid, and the doorbell) on walks and commutes. | [What I Look For in Wireless Earphones](/blog/what-i-look-for-in-wireless-earphones/) |
| I wear **glasses** and want **real buttons**, not touch controls, on earphones. | [What I Look For in Wireless Earphones](/blog/what-i-look-for-in-wireless-earphones/) |
| My everyday pair is **Soundpeats RunFree** (neckband, USB-C, open-ear). | [What I Look For in Wireless Earphones](/blog/what-i-look-for-in-wireless-earphones/) |
| I take **Google Meet** and **Messenger** calls on the same earphones I wear all day. | [What I Look For in Wireless Earphones](/blog/what-i-look-for-in-wireless-earphones/) |
| I **shop on Shopee** and filter for **local sellers** near sorting centers like Paranaque and Biñan. | [Shopee Tips](/blog/shopee-tips/) *(draft)* |
| I use **Inoreader** for RSS (free tier; most setups land around **20-30 feeds**). | [RSS Feeds - What I Wish I'd Done Sooner](/blog/getting-started-with-rss-feeds/) |
| I still need **Facebook for Messenger**; I mostly **stopped using the main feed** and check **Favorites** for people I picked. | [Facebook's Hidden Gem: The Favorites Feed](/blog/facebook-favorites-feed/) |
| I watch people **right-click to open tabs** when the **mouse wheel click** does the same thing on every OS. | [The Mouse Wheel Click](/blog/mouse-wheel-click/) |
| I **sit for work**; I keep **McGill's Big Three** in rotation for low-back maintenance. | [McGill's Big Three](/blog/mcgill-big-three/) *(draft)* |
| I keep a modest **supplement stack** when sleep and food slip - I can't control every virus on a **jeepney**. | [Personal Immune Booster Supplements](/blog/immune-booster-supplements/) *(draft)* |

---

## Acoustic guitar, VLC pads, and DNPAP (yes, bias)

| Fact | Where I said it |
| :--- | :--- |
| I **lead worship** with an **acoustic guitar** and run **MP3 pads through VLC** - not pad apps, after in-service freezes. | [Worship Pads for Solo Guitar](/blog/worship-pads-solo-guitar/) |
| **Church Wi-Fi** has betrayed me enough that offline **MP3 + VLC** is the setup I trust. | [Worship Pads for Solo Guitar](/blog/worship-pads-solo-guitar/) |
| VLC crossfades are simple enough that **I could hand it to a teenager in five minutes** if that scenario came up. | [Worship Pads for Solo Guitar](/blog/worship-pads-solo-guitar/) |
| **DNPAP** (Diyos Ng Pagasa at Pagibig) is a favorite local worship song - partly because **Pia recorded it**. Yes, bias. | [DNPAP - Song Resources](/blog/dnpap-song-resources/) |

---

## Alex Eala, quitting Marvel Snap, Hanamikoji rethemes

| Fact | Where I said it |
| :--- | :--- |
| I've followed **Alex Eala** since her **2025 Miami Open** run and keep a living career tracker on this site. | [Alex Eala Pro Career Tracker](/blog/alex-eala-pro-career-tracker/) |
| I **quit Marvel Snap** after cube anxiety and the meta treadmill - and **never spent a cent** on it. | [Why I Stopped Playing Marvel Snap](/blog/why-i-stopped-playing-marvel-snap/) |
| I'm **not a comics completionist** - Snap worked because I already knew the Marvel crowd without reading up. | [Why I Stopped Playing Marvel Snap](/blog/why-i-stopped-playing-marvel-snap/) |
| I love **Hanamikoji** for the **I split, you choose** mechanic, not the geisha art. | [Hanamikoji Duopoly Retheme](/blog/hanamikoji-duopoly-retheme/) |
| I rethemed it as **Duopoly** for home play - cut components, no retail art, private deck only. | [Hanamikoji Duopoly Retheme](/blog/hanamikoji-duopoly-retheme/) |
| I print **1-page zines** (eight pages from one letter-size sheet) for **board game rules** at home. | [One Sheet, Eight Pages](/blog/one-page-zine/) |

---

## A sanity check, not an identity document

For me this is a quick read to see if the facts still line up - same kid age, same stack, same worship setup, same earphones in the drawer.

If you're new here and want the human version before the essays, start with the rows that sound most like a person and click through. If Pia, Isaiah, the Instant Pot, and the VLC pads all show up in the linked posts, it's probably still me typing.

What it misses: opinions, half-finished drafts, and everything I never wrote down. This table is only what made it into a post.

---

## If something drifts

Blogs are not identity documents. Kids grow. Gear changes. Hosting stacks get swapped. I'll update this table when a fact goes stale - or leave the old row as a timestamp of who I was when I wrote it.

If you spot a row that does not match the linked post anymore, the blog aged. If a row matches the post but does not sound like me anymore, I aged. Either way, worth a rewrite.
