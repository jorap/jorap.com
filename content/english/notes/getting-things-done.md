---
title: "Getting Things Done"
meta_title: "Getting Things Done"
description: "David Allen's capture-clarify-organize loop gives me a trusted inbox, next actions, and a weekly review."
key_concept: |
  GTD vs PARA splits tasks from notes; Allen owns the task side.
  
  Allen task-inbox system: capture everything, clarify next actions, trust the weekly review.
  [[GTD vs PARA]] splits tasks from notes; Allen owns the task side.
  
  Capture everything, clarify next actions, trust the weekly review.
  
  The task inbox and the note garden are separate lanes - Allen owns tasks.
examples:
  - "Tournament week my brain isn't a filing cabinet - GTD was one trusted inbox and a weekly review so open loops lived on paper, not at 2 a.m."
  - "Commute errands and neighbor requests hit one pocket notebook - each item gets a next action so nothing rots as background worry."
shareable_lines:
  - "David Allen's capture-clarify-organize loop gives me a trusted inbox, next actions, and a weekly review."
  - "GTD vs PARA splits tasks from notes; Allen owns the task side."
  - "Allen task-inbox system: capture everything, clarify next actions, trust the weekly review."
  - "The task inbox and the note garden are separate lanes."
relationships:
  - type: contradicts
    wikilink: "[[Slow Productivity]]"
    reason: "when the opposite frame fits better"
  - type: extends
    wikilink: "[[GTD vs PARA]]"
    reason: "Allen owns the task side of the split"
  - type: extends
    wikilink: "[[The Trusted Inbox]]"
    reason: "Trusted inbox is the heart of the loop"
slug: "getting-things-done"
date: "2026-06-22 06:00:00+00:00"
image: "/images/note.jpg"
categories: ["Productivity"]
author: "JoRap"
tags: ["PKM", "Productivity", "GTD"]
featured: false
review: true
card_sets: ["Focus", "Workflow"]
cards:
  - front: "Tournament week and my brain is holding seventeen open loops at 2 a.m. What's the move?"
    back: "One trusted inbox - weekly review."
  - front: "School forms, dentist calls, permission slips piled on the counter. GTD first step?"
    back: "One kitchen inbox - capture everything."
  - front: "Neighbor texted three errands mid-commute and they're all sitting fuzzy in my head. One move per item?"
    back: "Next action on each - nothing stays fuzzy."
  - front: "Brain isn't a filing cabinet. Where do open loops live?"
    back: "Paper or app I trust - not my head at 2 a.m."
draft: false
---
