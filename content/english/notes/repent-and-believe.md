---
title: "Repent and Believe"
meta_title: "Repent and Believe - Jesus' First Command"
description: "Jesus' opening command: turn from self-rule and trust the good news that the kingdom is at hand."
key_concept: |
  - Kingdom's here now - turn from self-rule and trust the gospel today, not when habits look respectable.

  {{< bible ref="Mark 1:14-15" emphasize="15" >}}

  - Belief is reliance on Christ's promise - that trust receives eternal life; it is not mere intellectual agreement.
  - Repentance turns from sin for fellowship - not the price of receiving eternal life.
  - Under [[Free Grace]], faith receives the gift; turn-from-sin belongs with the walk, [[Standing vs Fellowship]], and grateful living under the King.
  - [[There Is No Perfect Solution]] - the kingdom is at hand now, not when you have fixed yourself.
level_1: "Repent and believe is Jesus' opening command - turn from self-rule and trust the gospel that the kingdom is at hand."
level_2: "Belief receives eternal life; repentance turns for fellowship and walk - respectable habits are fruit, not the door price."
level_3: "Trust the coach's system today, messy as you are - do not wait until your form looks respectable."
level_4: "Because the kingdom arrived in Christ, delay is another form of self-leadership - belief today receives the gift; turn keeps fellowship warm."
level_5: "Preach the pair with [[Free Grace]] order straight - faith receives eternal life; repentance serves fellowship and the thank-you walk."
examples:
  - "I kept telling myself I would trust the coach's system once my form looked respectable - faith receives the gift today, messy as you are."
  - "I kept telling myself I would trust Jesus once my habits looked respectable - eternal life is not a finish line when you are fixed."
shareable_thought:
  - "Jesus' opening command: turn from self-rule and trust the good news that the kingdom is at hand."
  - "Belief is reliance on Christ's promise - that trust receives eternal life."
  - "Repentance turns from sin for fellowship - not the price of receiving eternal life."
  - "Kingdom's here now - trust the gospel today, not when habits look respectable."
relationships:
  - type: contradicts
    wikilink: "[[There Is No Perfect Solution]]"
    reason: "when I delay entry until I have fixed myself"
  - type: extends
    wikilink: "[[Free Grace]]"
    reason: "Faith receives eternal life - turn-from-sin belongs with fellowship and walk"
  - type: extends
    wikilink: "[[Renewed Mind]]"
    reason: "Change of mind at entry; renewal keeps the turn going"
  - type: extends
    wikilink: "[[Standing vs Fellowship]]"
    reason: "Belief settles standing; repentance restores closeness after sin"
slug: "repent-and-believe"
date: "2026-06-19 09:15:00+00:00"
image: "/images/note.jpg"
categories: ["Faith"]
author: "JoRap"
tags: ["Gospel", "Jesus Christ", "Eternal Principles", "Free Grace", "Repentance", "Faith"]
aliases: ["repentance", "believe the gospel"]
featured: false
review: true
card_sets: ["Eternal Principles", "Faith"]
cards:
  - front: "Delaying faith until my habits improve. When do I enter?"
    back: "Today - by faith, not when fixed."
  - front: "Someone groveling until they feel good enough for God. One sentence I would say?"
    back: "Trust Christ now - stop earning it."
  - front: "Head agrees with gospel facts but life unchanged. What's missing?"
    back: "Rely on Christ's promise - trust."
  - front: "Trying to obey my way into eternal life. What receives life vs what guides after?"
    back: "Faith receives life - turn and obey guide the walk."
  - front: "Same sin again - despair that heaven is lost. What does turning again mean?"
    back: "Fix closeness with God - not re-earn heaven."
  - front: "Someone says repentance buys heaven and faith is just agreeing. Split the categories?"
    back: "Faith receives life - repentance for fellowship."
draft: false
---
